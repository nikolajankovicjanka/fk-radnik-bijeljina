        ...
        <template v-else>
            <header class="hero">
                ...
                <div class="hero-inner">
                    ...
                    <p v-if="item.excerpt" class="hero-lead">{{ item.excerpt }}</p>
            ...
            <section class="news-card-content">
                <div class="container">
                    <div v-if="item.content" class="rich" v-html="item.content"></div>
...
.hero-overlay {
    ...
    inset: 0;
    background:
            linear-gradient(180deg, rgba(0,0,0,0.20) 0%, rgba(0,0,0,0.70) 65%, rgba(0,0,0,0.88) 100%);
}
...
.rich :deep(p) { margin: 0 0 18px; }
.rich :deep(h2) { margin: 26px 0 10px; font-size: 1.6rem; }
.rich :deep(img) { max-width: 100%; border-radius: 12px; }

/* Responsive */
@media (max-width: 640px) {
    .hero { min-height: 480px; }
    .hero-inner { padding-bottom: 24px; }
    .time { display: none; } /* kao često na mobile */
}
</style>
