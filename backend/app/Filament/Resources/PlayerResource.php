...
class PlayerResource extends Resource
{
    ...
    public static function form(Form $form) : Form
    {
        return $form->schema([Section::make('Igrač')->columns(2)->schema([Select::make('team_type')->label('Selekcija')->options(TeamType::options())->required(),
                                                                          ...
                                                                          TextInput::make('shirt_number')->label('Broj dresa')->numeric()->minValue(0)->maxValue(99)->required()
                                                                              // unique po selekciji (team_type + shirt_number)
                                                                              ->rule(function (Forms\Get $get, ?Player $record) {
                                                                          ...
                                                                          FileUpload::make('photo')->label('Slika')->disk('public')->directory('players')->image()->imageEditor()->imageCropAspectRatio('3:4')->imageResizeTargetWidth('900')->imageResizeTargetHeight('1200')->maxSize(4096)->columnSpanFull()->helperText('Preporuka: 900x1200 (3:4).'),
